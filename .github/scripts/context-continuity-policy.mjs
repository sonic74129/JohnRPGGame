...
function extractMarkedBlock(content, sourceLabel) {
  ...
  const beginCount = countOccurrences(normalized, BEGIN_MARKER);
  const endCount = countOccurrences(normalized, END_MARKER);
  if (beginCount !== 1) {
    throw new Error(
      `${sourceLabel} must contain ${BEGIN_MARKER} exactly once; found ${beginCount}.`,
    );
  }
  if (endCount !== 1) {
    throw new Error(
      `${sourceLabel} must contain ${END_MARKER} exactly once; found ${endCount}.`,
    );
  }
